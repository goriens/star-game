...
const Level5: React.FC<{
  ...
  // ✅ updated signature to return scoring stats
  onLevelComplete: (
    success: boolean,
  ...
  const defaultConfig = {
    title: 'Level 5 – Star Seeker!',
    description: 'Now there are 16 icons and 6 hidden stars. Can you find them all?',
  }
  const { title, description } = config || defaultConfig

  // 12 shape types, 16 total icons, 6 stars
  const baseShapes = [
    { type: 'star' }, { type: 'triangle' },
    { type: 'square' }, { type: 'hexagon' },
    { type: 'heart' }, { type: 'circle' },
    { type: 'octagon' }, { type: 'diamond' },
    { type: 'pentagon' }, { type: 'cloud' },
    { type: 'star' }, { type: 'triangle' },
    { type: 'star' }, { type: 'star' },
    { type: 'star' }, { type: 'star' },
  ...
  // ---------- ✅ Level Complete Popup (now tracks results) ----------
  if (showPopup)
    return (
        ...
        <AnimatePresence>
          ...
          >
            ...
            <p className="text-lg text-gray-700">
              You found all 6 stars! That’s amazing focus!
            </p>
      ...
      <div className="grid grid-cols-4 gap-8 justify-items-center px-6 pb-10">
        {shapes.map((shape, i) => (
          ...
          >
            ...
            {shape.type === 'triangle' && <Triangle className="w-18 h-18 text-purple-400" />}
            {shape.type === 'square' && <Square className="w-18 h-18 text-blue-400" />}
            {shape.type === 'hexagon' && <Hexagon className="w-18 h-18 text-rose-400" />}
            {shape.type === 'heart' && <Heart className="w-18 h-18 text-pink-400 fill-pink-300" />}
            {shape.type === 'circle' && <Circle className="w-18 h-18 text-green-400" />}
            {shape.type === 'octagon' && <Octagon className="w-18 h-18 text-orange-400" />}
            {shape.type === 'diamond' && <Diamond className="w-18 h-18 text-cyan-400" />}
            {shape.type === 'pentagon' && <Pentagon className="w-18 h-18 text-indigo-400" />}
            {shape.type === 'cloud' && <Cloud className="w-18 h-18 text-gray-400" />}
            {shape.hasSticker && (
